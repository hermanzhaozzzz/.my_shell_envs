if not set -q CLASHCTL_HOME
    # MSE sets CLASHCTL_HOME from the repository-managed zsh configuration.
    # If absent, the repository deploy did not finish — bail out quietly.
    exit 0
end
...
function __clashctl_state_path
    echo $CLASHCTL_HOME/state/proxy.fish
end
